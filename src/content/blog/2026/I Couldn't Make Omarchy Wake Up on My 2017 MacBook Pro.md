...
- Persistent `s2idle` made resume usable at the cost of power consumption

That is enough to choose a safe operating policy without inventing certainty. Debugging is not always a path to a clean fix. Sometimes the valuable result is knowing which explanations the evidence rules out, where the risk remains, and when to stop changing a working machine.
